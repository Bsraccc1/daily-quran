...
class UserPreferences @Inject constructor(
    ...
    /**
     * Persist `pagesRead` on the active session, but only if the
     * new value is **strictly greater than** the existing value
     * (monotonic watermark) and **at most** the session's
     * `targetPages` (capped).
     *
     * Routes through [SessionProgressMath.nextPagesReadWatermark]
     * to keep the rules in one place. See that function's KDoc for
     * the full invariant rationale.
     *
     * Pre-fix bug: the old version always overwrote `pagesRead`,
     * so the auto-save firing on backward navigation (e.g. user
     * goes from page 20 back to page 5 to re-read) would erase the
     * 20-page mark. The Dashboard's "Continue Reading" button
     * keys off `pagesRead`, so the user got bounced back to page 5
     * after a restart.
     *
     * @param newRaw the candidate `pagesRead` computed by the caller
     *               (typically `currentPage - startPage + 1`,
     *               coerced to ≥ 0 by the caller).
     */
    suspend fun updateSessionProgress(newRaw: Int) = dataStore.edit { prefs ->
        ...
        val next = SessionProgressMath.nextPagesReadWatermark(
            existing = session.pagesRead,
            newRaw = newRaw,
            targetPages = session.targetPages,
        ) ?: return@edit
        current[idx] = session.copy(pagesRead = next)
        prefs[SESSIONS_KEY] = serializeSessions(current)
    }
